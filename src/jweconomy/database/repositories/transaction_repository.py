...
class TransactionRepository:
    ...
    async def record_transaction(
        ...
        cursor = await self._db.execute(
            """
            INSERT INTO transactions (sender_uuid, receiver_uuid, amount, tax_amount, transaction_type, description)
            VALUES (?, ?, ?, ?, ?, ?)
            """,
            (sender_uuid, receiver_uuid, amount, tax_amount, transaction_type, description),
        )
        return cursor.lastrowid

    async def get_player_transactions(
        self, uuid: str, limit: int = 20, offset: int = 0
    ...
    async def get_transaction_count(self, uuid: str) -> int:
